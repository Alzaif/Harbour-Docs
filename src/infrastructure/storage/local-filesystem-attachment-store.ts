...
import type { DocsDatabase } from '../persistence/create-database.js';
...
export class LocalFilesystemAttachmentStore implements AttachmentStorePort {
  ...
  async deleteByDocument(ownerUserId: string, documentId: string): Promise<void> {
    const rows = await this.db
      .select()
      .from(attachments)
      .where(
        and(eq(attachments.documentId, documentId), eq(attachments.ownerUserId, ownerUserId)),
      );
